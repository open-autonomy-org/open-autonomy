// A standalone in-memory HTTP example: health, validation, CRUD and search.
  ...
  return Bun.serve({
    ...
    async fetch(req) {
      ...
      if (req.method === 'GET' && url.pathname === '/notes') {
        ...
        return Response.json(q ? notes.filter((n) => n.text.toLowerCase().includes(q)) : notes);
      }
      ...
      if (one) {
        ...
        if (req.method === 'DELETE') { notes.splice(at, 1); return new Response(null, { status: 204 }); }
      }
...
if (import.meta.main) {
  ...
  console.log(`notes-http listening on ${server.url}`);
}
