...
- **What this is.** A todo list CLI in TypeScript on bun, with an Open Autonomy agent (`hermes/`, applied from the template). The board is what you build next, in order; `STANDARDS.md` is the bar. `src/cli.ts` is the program; `test/` is its proof.
