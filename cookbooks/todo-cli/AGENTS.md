...
- **What this is.** A todo list CLI in TypeScript on bun, with an Open Autonomy agent (`hermes/`, applied from the template). `ROADMAP.md` is the sourced planning memory, maintained by the PM scrum; the kanban holds fleet execution; `CONTRIBUTING.md` is how code is written here. `src/cli.ts` is the program; `test/` is its proof.
