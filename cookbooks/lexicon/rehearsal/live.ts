#!/usr/bin/env bun
// A deployed lexicon service in the world (rehearsal/world.json names it): it reports the commit it is running on Hookline's `GET /api`
...
Bun.serve({
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/api') return reachable ? Response.json({ name: 'lexicon', version: commit ?? null }) : new Response('unreachable', { status: 503 });
