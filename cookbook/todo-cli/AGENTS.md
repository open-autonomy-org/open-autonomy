...
- **Checks.** `bun run check` from the repository root runs the tests and the typecheck. It must pass before every push. Every item adds tests for what it adds; keep `HELP` in `src/cli.ts` naming every command.
