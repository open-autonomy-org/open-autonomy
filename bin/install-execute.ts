...
export function stepInstallDeps(sel: SelectionRecordRef, opts: { proc: ProcRunner; detectFile?: string; dryRun?: boolean }): ExecuteStepResult {
  ...
  if (!ztrackPresent) {
    if (opts.dryRun) {
      wouldInstall.push('npm install -D ztrack@1.4.0');
      notes.push('[DRY-RUN] ztrack absent — would run: npm install -D ztrack@1.4.0 (NOT run; node_modules/package.json untouched)');
    } else {
      const r = opts.proc('npm', ['install', '-D', 'ztrack@1.4.0'], { cwd: repoDir });
      if (r.status !== 0) return step('install-deps', 'blocked', `npm install -D ztrack@1.4.0 failed (exit ${r.status}): ${firstLine(r.stderr || r.stdout)}`);
      notes.push('installed ztrack@1.4.0 (was absent)');
    }
  } else {
