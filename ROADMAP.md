...
- Owner-authored PR #611 proposes kit 2.9.0, SDK 2.5.0 and backend 0.2.0 for the operating-state outcome below. Head `fda82472` is independently approved, but an owner review on its superseded head still blocks merge, so its scope and version bumps are not part of the release candidate.
...
- The owner dismisses stale review 5184630564 on superseded head `3d24d124`; then auto-merge lands approved head `fda82472` and the native reviewer confirms its ancestry on main. Keep the live-run pause window explicit because it remains source-inspected rather than exercised. Release, publication and deployment remain separate gates.
