...
- PRs #509 and #521 prevent fleet activation on the host and verify the installed signed-in account/model control plane without copying authentication, but the intended host sidecar/container executor, disconnect behavior, one real model turn, in-container Hermes callbacks and host-side reporting remain unimplemented or unverified.
...
Status: activation fails closed; the installed signed-in account and selected model are verified read-only, but the intended isolated runtime and a model turn are not implemented or end-to-end verified.
Dispatch: hold

Source: [PR #509](https://github.com/open-autonomy-org/open-autonomy/pull/509), which landed the safety guard, and [PRs #521–#522](https://github.com/open-autonomy-org/open-autonomy/pull/522), which verified the native account/model control plane and clarified the remaining startup boundary.
...
- The already verified native initialization and account/model access are followed by one bounded turn through the isolated runtime, then in-container PM/worker tools and host-side reporting. Synthetic providers and empty Codex homes do not establish activation.
