...
- PRs #509, #521, #523 and #527 prevent fleet activation on the host, verify the installed signed-in account/model control plane without copying authentication, and bound native forwarding to a pinned remote executor that fails closed. A synthetic World-managed container command verified host-file and credential exclusion, but the host runner, protected connection, full in-container Hermes/MCP tools, host-side reporting and one real subscription turn remain incomplete or unverified; the next image build failed for lack of Docker VM disk.
...
Status: activation fails closed; the installed signed-in account/model and a restricted remote-executor bridge are verified, including one synthetic command inside a hardened container, but the complete host-runner/container fleet and a real subscription turn are not implemented or end-to-end verified.
Dispatch: hold

Source: [PR #509](https://github.com/open-autonomy-org/open-autonomy/pull/509), which landed the safety guard; [PRs #521–#522](https://github.com/open-autonomy-org/open-autonomy/pull/522), which verified the native account/model control plane and clarified startup; and [PRs #523 and #527](https://github.com/open-autonomy-org/open-autonomy/pull/527), which bounded native forwarding and exercised Codex's trusted-host external-sandbox policy through a hardened container.

Completion:
- Complete the host runner and protected container connection so Hermes, its workspace and every agent tool run inside the container while the host retains installed Codex authentication and reporting.
- Preserve the verified bridge properties: pinned model/workspace/remote executor, unavailable account/configuration/history APIs, host-only external-sandbox selection, restricted network and disconnect without host fallback; prove them with the full fleet rather than only the synthetic command.
- Follow the already verified native initialization and account/model access with one bounded real subscription turn, then in-container PM/worker skills, Kanban and communication tools plus host-side reporting. Synthetic providers do not establish activation.

Dependency: World memory admission now succeeds and the bounded container command ran, but building the pinned upstream Hermes image failed with `No space left on device` inside the selected `colima-rh2local` Docker VM. The complete fleet proof needs adequate Docker storage before dispatch.
