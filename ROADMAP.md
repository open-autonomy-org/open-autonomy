...
- Owner-authored PR #611 proposes kit 2.9.0, SDK 2.5.0 and backend 0.2.0 for the operating-state outcome below. Head `fda82472` received changes-requested review because its manual evidence does not exercise the ambiguous pause failure boundary, so its scope and version bumps are not part of the release candidate.
...
Status: owner-authored implementation and ADR 0003 remain open in PR #611. Exact-head review confirms the write-ahead code addresses the source defect, but requests a manual exercise across the actual ambiguous pause-operation failure/restart boundary before acceptance.
Dispatch: hold

Source: verified owner `yueranyuan` authored and signed all three commits in [PR #611](https://github.com/open-autonomy-org/open-autonomy/pull/611); proposed [ADR 0003](https://github.com/open-autonomy-org/open-autonomy/blob/fda82472abc290e8f5165082d4d44b2cf1003c0f/docs/decisions/0003-operating-state-through-the-sdk.md) separates the owner's desired state from the automation's observed state. The ADR's quoted private conversation is not used as authority evidence. [App review 5184632769](https://github.com/open-autonomy-org/open-autonomy/pull/611#pullrequestreview-5184632769) records the prior-head source defect; commit [`fda82472`](https://github.com/open-autonomy-org/open-autonomy/commit/fda82472abc290e8f5165082d4d44b2cf1003c0f) writes ownership ahead of the mutation, while [exact-head review 5185995953](https://github.com/open-autonomy-org/open-autonomy/pull/611#pullrequestreview-5185995953) records the remaining manual-evidence gap in `hermes:task/t_fb86081f`.
...
- Manually drive an ambiguous pause outcome through the product path: the ownership save must survive a pause operation that applies and then fails, or an interruption before successful completion is observed; after restart, verify the owned job remains paused and a running request resumes only it and clears ownership. Then obtain fresh exact-head review. Keep the live-run pause window explicit because it also remains source-inspected rather than exercised. Release, publication and deployment remain separate gates.
