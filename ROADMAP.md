...
Status: the 2.8.2 proposal is withdrawn. The operating-state work and SDK-interface replacement are now on main after independent review, including PR #628's atomic update-index follow-up. Candidate preparation remains held on artifact/runtime verification, the separately reviewed PR #622 provenance repair, correction of PR #632's repeated owner-review merge, and the Storybook dependency advisory still reported by Security after PR #634.
Dispatch: hold
...
- PR #618 landed after exact-head approval in `hermes:task/t_76e970b3`. The verified owner closed PR #619–#621 unmerged as superseded by PR #624 in comments [5648622820](https://github.com/open-autonomy-org/open-autonomy/pull/619#issuecomment-5648622820), [5648622958](https://github.com/open-autonomy-org/open-autonomy/pull/620#issuecomment-5648622958) and [5648623124](https://github.com/open-autonomy-org/open-autonomy/pull/621#issuecomment-5648623124). Corrected PR #624 proposes kit 2.10.0, SDK 3.0.0 and backend 0.3.0 but remains changes-requested in `hermes:task/t_57c269c4`; until a revised exact head passes review and merges, those versions and effects are not a candidate.
- Owner-authored PR #622 merged after the owner also submitted approval 5188012108; the landing workflow's bot PR identity does not make that review independent. Release remains held while `hermes:task/t_ec574a74` audits the exact head and identifies a supported correction without claiming retroactive approval.
- PRs [#629](https://github.com/open-autonomy-org/open-autonomy/pull/629), [#633](https://github.com/open-autonomy-org/open-autonomy/pull/633) and [#634](https://github.com/open-autonomy-org/open-autonomy/pull/634) merged the excluded, unwired Storybook project, directory and account designs after exact-head App approval. Security remains red through [run 34725807604](https://github.com/open-autonomy-org/open-autonomy/actions/runs/34725807604); the unchanged lock still carries Storybook's transitive `@vitest/mocker` affected by `GHSA-82fw-gwwq-j7x9`. The supply-chain workflow marks dependency changes human-required; a candidate that contains these merges needs a sourced correction and a green Security result.
- PR #631 correctly reverted PR #630's planning effect after exact-head App approval, but PR #632's exact re-land then merged as `577eb2be` on owner-user review [5188429781](https://github.com/open-autonomy-org/open-autonomy/pull/632#pullrequestreview-5188429781) with no pre-merge App approval. Native task `hermes:task/t_34dec337` correctly refused to claim the gate; audit `hermes:task/t_4a892dbb` prescribed a new separately App-reviewed bounded revert and re-land that preserve PRs #633–#635. Release remains held until both corrective merges land.
...
Status: audit confirmed the supported planning content merged without the required prospective GitHub App approval. A new bounded correction is ready for fleet execution; the original owner-user review remains historical evidence, not agent approval.
Dispatch: fleet

Source: merged [PR #632](https://github.com/open-autonomy-org/open-autonomy/pull/632), owner-user review [5188429781](https://github.com/open-autonomy-org/open-autonomy/pull/632#pullrequestreview-5188429781), blocked native task `hermes:task/t_34dec337`, and independent post-merge audit `hermes:task/t_4a892dbb`.

Completion:
- From fresh current main, reverse only PR #632's mainline-parent effect in ROADMAP.md and CHANGELOG.md while preserving PR #633/PR #634 page work and replaying PR #635's later reconciliation; a fresh-context agent submits and reads back exact-head GitHub App approval before the revert merges.
...
- Confirm both reviewed heads in main. Preserve PR #631's valid review, PR #632's owner review and PR #635's valid review with their actual scopes; do not describe any later review as retroactive approval of PR #632. Release remains held until both corrections land.
...
Source: the verified owner's [PR #593 ruling](https://github.com/open-autonomy-org/open-autonomy/pull/593#issuecomment-5628311527) was the original contrary public evidence. Accepted [ADR 0001](docs/decisions/0001-runtime-boundary.md) explicitly replaces that conflicting instruction and PR #594; verified owner `yueranyuan` then affirmed that replacement in [comment 5648467119](https://github.com/open-autonomy-org/open-autonomy/pull/594#issuecomment-5648467119). [PR #597](https://github.com/open-autonomy-org/open-autonomy/pull/597) is the merged implementation, approved at exact head `b13099c0`; corrected PR #624 proposes relocating the same host tools between packages without changing the trust boundary.
...
Status: PR #618 landed after exact-head review. The verified owner closed changes-requested PR #619–#621 as superseded by corrected PR #624. PR #624 adds proposed ADR 0005 and fixes several predecessor findings, but its exact-head review still requires crash/lost-ack idempotency, a valid public wire example and fresh manual evidence.
Dispatch: hold

Source: signed owner commits at landed [PR #618](https://github.com/open-autonomy-org/open-autonomy/pull/618), superseded [PR #619](https://github.com/open-autonomy-org/open-autonomy/pull/619), [PR #620](https://github.com/open-autonomy-org/open-autonomy/pull/620), [PR #621](https://github.com/open-autonomy-org/open-autonomy/pull/621) and corrected [PR #624](https://github.com/open-autonomy-org/open-autonomy/pull/624); owner supersession comments 5648622820/5648622958/5648623124; exact-head review scopes `hermes:task/t_76e970b3`, `hermes:task/t_115575ae`, `hermes:task/t_e93e9c81`, `hermes:task/t_7acecc4b` and `hermes:task/t_57c269c4`; PR #624 App review 5188046297.

Completion:
- Landed in PR #618: reserve `paused` for the owner's operating state and call a funded balance spent to zero `exhausted` without changing the ledger's spending behavior.
