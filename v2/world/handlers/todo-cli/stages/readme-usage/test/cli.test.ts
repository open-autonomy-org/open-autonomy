...
let dir: string;
beforeEach(() => { dir = mkdtempSync(join(tmpdir(), 'todo-')); process.env.TODO_FILE = join(dir, 'todo.json'); process.env.TODO_TODAY = '2026-09-04'; });
...
test('due dates are stored, sort first in list, and flag overdue items against $TODO_TODAY', () => {
  main(['add', 'later']); main(['add', 'pay rent', '--due', '2026-10-01']); main(['add', 'taxes', '--due', '2026-04-15']);
  expect(store().items[1].due).toBe('2026-10-01');
  expect(main(['list']).out).toBe('3  taxes (due 2026-04-15) OVERDUE\n2  pay rent (due 2026-10-01)\n1  later\n');
  expect(main(['add', 'x', '--due', 'soon']).code).toBe(2);
});

test('stats counts open, done and overdue items', () => {
  main(['add', 'a']); main(['add', 'b', '--due', '2020-01-01']); main(['add', 'c']); main(['done', '3']);
  expect(main(['stats']).out).toBe('open 2  done 1  overdue 1\n');
});

test("the README's usage section is the help text", () => {
  const readme = readFileSync(new URL('../README.md', import.meta.url), 'utf8');
  const block = /## Usage\n\n```\n([\s\S]*?)```/.exec(readme)?.[1];
  expect(block).toBe(HELP);
});
