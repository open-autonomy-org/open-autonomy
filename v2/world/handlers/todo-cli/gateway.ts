#!/usr/bin/env bun
// The model's side of build-roadmap runs on cookbooks/todo-cli, printed as the openai twin's scenario JSON
...
const handlers = [
  ...
  { id: 'after-failed-implementation', on: { anyTextIncludes: 'IMPLEMENTATION_RAN' }, respond: { text: 'Stopped: the implementation ran but did not push; its output is above. Nothing was recorded on the roadmap.' } },
  ...items.map((item) => ({ id: `implement-${item.id}`, on: { anyTextIncludes: `NEXT_ITEM=${item.id}` }, respond: { toolCalls: { name: 'terminal', arguments: { command: implement(item) } } } })),
  { id: 'nothing-open', on: { anyTextIncludes: 'NEXT_ITEM=none' }, respond: { text: 'Nothing to do: every item on ROADMAP.yml is done.' } },
  {
