...
- `hermes-live-view` (partial): the project page carries the agent's spine — NEXT (the schedule and the top open item), NOW (the running job, streaming its turns over SSE) and DONE (one receipt per finished run: item, duration, report, commit) — with a run page per job and a `now.svg` widget. Runs arrive as CloudEvents on the standing key from `agent-reporter.ts`, which reads the Hermes home through supercode's `sessions.index.subscribe` + `sessions.follow` (added to supercode for Hermes in volter-ai/supercode#692). Still open: rendering the setup itself (SOUL, skills, config).
