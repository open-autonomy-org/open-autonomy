...
export const WorkingNow: S = { args: base(openAutonomy, { live: [pmTail.key], tail: pmTail, sessions: [{ ...openAutonomy.sessions.find((s) => s.key === pmTail.key)!, status: 'live', ended_at: undefined, outcome: undefined, started_at: new Date(NOW - 4 * 60_000).toISOString() }, ...openAutonomy.sessions.filter((s) => s.key !== pmTail.key)] }) };
