...
- **The world.** `world/` is the whole product on your machine with no keys: the real worker, twins of GitHub and the model gateway (`world/README.md`). `bun world/run.ts up` runs it, `env -- <cmd>` reaches it, `down --purge` forgets it. Verify acceptance there; you cannot reach production.
