#!/usr/bin/env bun
// GitHub Actions for the twin, played by the world. The twin holds pull requests, required checks, branch
// protection and auto-merge, and lands a merge on its git wire when the required check is green, but runs
// no workflows. This process plays the runner for the landing convention the kit prescribes:
...
const gh = api(github);
const repos = (process.env.WORLD_ACTIONS_REPOS ?? ACCOUNT).split(',').map((s) => s.trim()).filter(Boolean);
...
async function tick(repo: string): Promise<void> {
  ...
  if (branches.status !== 200) return;
  const pulls = await gh.get(`/repos/${repo}/pulls?state=all&per_page=100`);
  const prs = (pulls.body ?? []) as Array<{ number: number; state: string; merged: boolean; head: { ref: string; sha: string } }>;
  for (const b of branches.body as Array<{ name: string; commit: { sha: string } }>) {
    ...
    const created = await gh.post(`/repos/${repo}/pulls`, { title, head: b.name, base: 'main', body: `Opened by the landing workflow for \`${b.name}\`. It merges on its own when the required checks pass.` });
  ...
  for (const p of prs.filter((p) => p.state === 'open' && !p.merged)) {
    ...
    if (((runs.body?.check_runs ?? []) as Array<{ name: string }>).some((c) => c.name === 'ci')) continue;
