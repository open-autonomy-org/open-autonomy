...
const payDomain = [
    ...
    `auth=$(curl -sf -u sk_test_world: -X POST http://127.0.0.1:47616/v1/test_helpers/issuing/authorizations -d card=$card_id -d amount=200 -d 'merchant_data[category]=computer_software_stores' -d 'merchant_data[name]=Namecheap') || { echo "PAYMENT_""RAN the registrar could not authorize"; exit 1; }`,
    ...
    `curl -sf -u sk_test_world: -X POST http://127.0.0.1:47616/v1/test_helpers/issuing/authorizations/$auth_id/capture >/dev/null || { echo "PAYMENT_""RAN the capture failed"; exit 1; }`,
...
const handlers = [
  ...
  { id: 'owner-block-say', on: { userTextIncludes: 'automatic task-status notification', toolResultFor: 'terminal' }, respond: { text: 'Set the `TODO_CLI_RELEASE_TOKEN` secret on the repository Actions secrets page. Then, from the project checkout, unblock the waiting card with `hermes kanban unblock "$(hermes kanban list --status blocked --json | sed -n \'s/^ *"id": *"\\([^"]*\\)".*/\\1/p\' | head -1)"`; that releases the worker.' } },
