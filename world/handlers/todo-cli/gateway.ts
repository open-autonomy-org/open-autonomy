...
function implement(item: { id: string; title: string }): string {
  ...
  const purchase = item.id === 'domain' ? [
    ...
    `card_id=$(printf '%s' "$card" | sed -n 's/.*"id":"\\([^"]*\\)".*/\\1/p' | head -1); [ -n "$card_id" ] || { echo "IMPLEMENTATION_""RAN no card id: $card"; exit 1; }`,
    ...
    `auth_id=$(printf '%s' "$auth" | sed -n 's/.*"id":"\\([^"]*\\)".*/\\1/p' | head -1); printf '%s' "$auth" | grep -q '"approved":true' || { echo "IMPLEMENTATION_""RAN the authorization was declined: $auth"; exit 1; }`,
