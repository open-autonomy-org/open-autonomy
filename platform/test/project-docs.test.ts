...
import { spineHtml, jobPageHtml, renderNowSvg, parseSchedule, renderSetupPanel, parseAgentConfig } from '../src/agent-view.js';
...
describe('the setup pane: who the agent is and how it runs, read from hermes/', () => {
  ...
  test('states the model, the schedule and the skills, and excerpts SOUL and the README with file links', () => {
    ...
    expect(html.includes('zai/glm-5.3-flash')).toBe(true);
    ...
    expect(html.includes('complete Hermes home')).toBe(true);
    expect(html.includes('Run it')).toBe(false); // headings are skipped, the file link carries the rest
    expect(html.includes('https://github.com/o/r/blob/HEAD/hermes/SOUL.md')).toBe(true);
    expect(html.includes('https://github.com/o/r/blob/HEAD/hermes/README.md')).toBe(true);
    expect(html.includes('https://github.com/o/r/blob/HEAD/hermes/config.yaml')).toBe(true);
  ...
  test('parses only the model block and renders nothing without a setup', () => {
    expect(parseAgentConfig(CONFIG)).toEqual({ model: 'zai/glm-5.3-flash', provider: 'custom' });
