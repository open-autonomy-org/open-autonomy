...
describe('agent model proxy', () => {
  ...
  test('routes a vendor/slug model over /v1/messages to the model gateway (no table entry) and settles on reported cost', async () => {
    const env = testEnv();
    // zai/glm-5.3-flash is NOT in the price table — it routes by the "vendor/slug" convention.
    const minted = await mint(env, ['zai/glm-5.3-flash'], 25, 5);
    ...
    const proxied = await worker.fetch(new Request('https://proxy.test/v1/messages', {
      ...
      body: JSON.stringify({ model: 'zai/glm-5.3-flash', max_tokens: 1000, messages: [] }),
    }), env, ctx);
  ...
  test('a /v1/messages call for an gateway model with no MODEL_GATEWAY_API_KEY is provider_not_configured', async () => {
    ...
    const proxied = await worker.fetch(new Request('https://proxy.test/v1/messages', {
      method: 'POST',
      headers: { authorization: `Bearer ${minted.token}`, 'content-type': 'application/json' },
      body: JSON.stringify({ model: 'zai/glm-5.3-flash', max_tokens: 100, messages: [] }),
    }), env, ctx);
  ...
  test('routes a vendor/slug model on /v1/chat/completions to the model gateway (no table entry) and settles on reported cost', async () => {
    const env = testEnv();
    // zai/glm-5.3-flash is NOT in the price table — the agent loop's proxyTurn hits this wire.
    const minted = await mint(env, ['zai/glm-5.3-flash'], 25, 5);
  ...
  test('forwards a large output cap unchanged and injects the ceiling when the client sends none', async () => {
    ...
    const env = testEnv();
    const minted = await mint(env, ['zai/glm-5.3-flash'], 100, 5);
    ...
    for (const body of [{ max_tokens: 32_000 }, {}, { max_tokens: 1_000_000 }]) {
      const res = await worker.fetch(new Request('https://proxy.test/v1/chat/completions', {
        method: 'POST',
        headers: { authorization: `Bearer ${minted.token}`, 'content-type': 'application/json' },
        body: JSON.stringify({ model: 'zai/glm-5.3-flash', messages: [], ...body }),
      }), env, ctx);
...
describe('self-serve project keys (claim file → standing key → rotate)', () => {
  ...
  const chat = (env: Env, token: string) => worker.fetch(new Request('https://proxy.test/v1/messages', {
    method: 'POST',
    headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    body: JSON.stringify({ model: 'zai/glm-5.3-flash', max_tokens: 5, messages: [] }),
  }), env, ctx);
  ...
  test('a repo carrying its claim mints a standing key that meters to its own account', async () => {
    ...
    expect(minted.run.models).toEqual(['zai/glm-5.3-flash']);
    expect((await chat(env, minted.token)).status).toBe(200);
  ...
  test('only a standing key can rotate, and an account holds at most three', async () => {
    const env = keyEnv();
    const run = await mint(env, ['zai/glm-5.3-flash'], 100, 10, { repo: ACCOUNT });
...
describe('agent jobs (the reporter narrates a run on the standing key, as CloudEvents)', () => {
  const standing = (env: Env, repo = 'volter/twin') => requestJson(env, '/admin/runs/mint', {
    ...
    body: { repo, issue: 0, actor: 'hermes', purpose: 'hermes', standing: true, models: ['zai/glm-5.3-flash'] },
  });
  let n = 0;
  ...
  test("the account is the key's own; a plain run token, a non-CloudEvent, or an unknown job is refused", async () => {
    ...
    const plain = await mint(env, ['zai/glm-5.3-flash'], 100, 10, { repo: 'acme/app' });
