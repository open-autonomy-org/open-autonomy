...
| `GET /v1/accounts/:account/statements` · `…/statements/:id/revisions` · `…/statements/:id/badges.svg` | the live statements, every change of one newest first, and its badge row for a README; behind the owner's `statements` panel (public unless the project is private). `statements`, `statementRevisions` |
