...
| `GET /v1/orgs/:org` | an org at a glance: its own `desired`, its `bounds` (the `spend.limits` of `<org>/.github/.open-autonomy/config.yaml`, used by every project of the org together), and each project whose overview is open to everyone, with its effective `control`, its balance, burn, runway and funding only when its books are open to everyone, and its live sessions only when its sessions are |
