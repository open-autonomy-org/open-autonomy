  ...
  try {
    ...
    for (const p of projects) {
      ...
      for (const profile of Object.keys(setup.profiles).filter((n) => n !== 'default')) {
        ...
        // serves, and the nested name left as a link to it: the project's reporter, which watches the project's
        // home, reads this profile's sessions through it, and every later sync of hermes/ writes through it.
        const script = 'if [ -L "$1" ]; then exit 0; fi; mkdir -p "$2" && cp -a "$1/." "$2/" && rm -rf "$1" && ln -s "../../$3" "$1"';
