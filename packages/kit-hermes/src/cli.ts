...
const verbs = new Set(['create', 'adopt', 'check', 'upgrade', 'setup']);
const verb = verbs.has(argv[0]) ? argv[0] : 'create';
const dir = argv.filter((a, i) => !a.startsWith('--') && argv[i - 1]?.startsWith('--') !== true && a !== verb)[0];
if (!dir) { console.error('usage: create-open-autonomy [create|adopt] <dir> --project <name> --account <owner/repo> | check <dir> | upgrade <dir> | setup <dir> [--plan]'); process.exit(2); }
