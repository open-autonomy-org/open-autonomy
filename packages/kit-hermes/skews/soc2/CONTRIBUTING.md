...
In this `soc2` project every ADR also answers the SOC 2 checklist
(`docs/decisions/SOC2-CHECKLIST.md`) under a `## SOC 2 checklist` heading: one line per item, C1 to C10, saying how
the item holds after the change or why the change does not touch it. The independent review rejects a record
whose answers the implementation does not bear out. `create-open-autonomy check` refuses a record missing an item.
The weekly internal audit (the `internal-audit` skill) re-checks the whole system against the same items.
