...
export async function handleModelCall(req: Request, env: Env, claims: KeyClaims, ctx: ExecutionContext, route: Route): Promise<Response> {
  ...
  const reservation = await ledger.reserve(requestId, claims.account, claims.kid, reserved, Number(env.MAX_GLOBAL_DAILY_USD_CENTS ?? 5000), model, estimateInputTokensFromBody(bodyText) + outputTokens, 'model', undefined, session);
  if (!reservation.ok) return error(reservation.error, reservation.error === 'auth_failed' ? 401 : reservation.error === 'rate_limit_reached' ? 429 : 402, { account: claims.account, message: reservation.message, balance_usd_cents: reservation.balance_usd_cents, earmarked_usd_cents: reservation.earmarked_usd_cents, reserved_usd_cents: reservation.reserved_usd_cents, available_usd_cents: reservation.available_usd_cents, needed_usd_cents: reservation.needed_usd_cents, ...(reservation.limit ? { limit: reservation.limit, used: reservation.used, needed: reservation.needed, current: reservation.current, retry_after_seconds: reservation.retry_after_seconds, how: reservation.how, ...(reservation.org ? { org: reservation.org } : {}) } : {}) });
