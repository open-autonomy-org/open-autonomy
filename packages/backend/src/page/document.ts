...
// shows (its drawing as a card, when the page is public), and the project's feed when it has one.
export interface PageMeta { description?: string; feed?: string; image?: string }
export function headMeta(title: string, brand: string, meta: PageMeta = {}): string {
  ...
  return `<title>${esc(full)}</title>${d ? `<meta name="description" content="${d}">` : ''}<meta property="og:type" content="website"><meta property="og:site_name" content="${esc(brand)}"><meta property="og:title" content="${esc(title)}">${d ? `<meta property="og:description" content="${d}">` : ''}${meta.image ? `<meta property="og:image" content="${esc(meta.image)}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta name="twitter:image" content="${esc(meta.image)}">` : ''}<meta name="twitter:card" content="${meta.image ? 'summary_large_image' : 'summary'}"><meta name="twitter:title" content="${esc(title)}">${d ? `<meta name="twitter:description" content="${d}">` : ''}${meta.feed ? `<link rel="alternate" type="application/atom+xml" title="${esc(title)} updates" href="${esc(meta.feed)}">` : ''}`;
}
