    ...
    <Shell d={d} current="agent">
      <div class="cols" style="grid-template-columns:minmax(0,1fr) 340px">
        ...
        <div class="side">
          ...
          <div class="card"><h2>Runs on</h2><ul class="rows"><li class="row"><span class="t">{d.v.profile.agent_harness ?? 'its harness'}</span><span class="n">{d.v.profile.agent_model ?? 'a model'}{d.v.profile.agent_provider ? ` · ${d.v.profile.agent_provider}` : ''}</span></li>{schedule.map((j) => <li class="row"><span class="t">{j.name ?? 'job'}</span><span class="n">fires {j.schedule ?? '?'}</span></li>)}{skills.length ? <li class="row"><span class="t">knows</span><span class="n">{skills.join(' · ')}</span></li> : null}{rt?.mode ? <li class="row"><span class="t">{rt.mode === 'container' ? 'in a container' : 'bare on a host'}</span><span class="n">{[rt.executor, rt.host].filter(Boolean).join(' · ') || '—'}</span></li> : null}{rt?.kit ? <li class="row"><span class="t">kit</span><span class="n">hermes {rt.kit}</span></li> : null}</ul></div>
