...
export async function mintCard(req: Request, env: Env, claims: KeyClaims): Promise<Response> {
  ...
  const reserved = await ledger.reserve(requestId, claims.account, claims.kid, body.usd_cents as number, Number(env.MAX_GLOBAL_DAILY_USD_CENTS ?? 5000), '', 0, 'card', item);
  if (!reserved.ok) return error(reserved.error, 402, { account: claims.account, message: reserved.message, balance_usd_cents: reserved.balance_usd_cents, earmarked_usd_cents: reserved.earmarked_usd_cents, reserved_usd_cents: reserved.reserved_usd_cents, available_usd_cents: reserved.available_usd_cents, needed_usd_cents: reserved.needed_usd_cents });
...
export async function settlePartner(req: Request, env: Env, claims: KeyClaims): Promise<Response> {
  ...
  const requestId = crypto.randomUUID();
  const reserved = await ledger.reserve(requestId, claims.account, claims.kid, body.usd_cents as number, Number(env.MAX_GLOBAL_DAILY_USD_CENTS ?? 5000), '', 0, 'partner', item);
  if (!reserved.ok) return error(reserved.error, 402, { account: claims.account, message: reserved.message, balance_usd_cents: reserved.balance_usd_cents, earmarked_usd_cents: reserved.earmarked_usd_cents, reserved_usd_cents: reserved.reserved_usd_cents, available_usd_cents: reserved.available_usd_cents, needed_usd_cents: reserved.needed_usd_cents });
