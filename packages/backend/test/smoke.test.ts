...
describe('the backend, one smoke test per surface', () => {
  ...
  test('the board: a task published under its item shows on the item, lane, attempts and verdict', async () => {
    ...
    expect(await (await request(env, '/acme/app/items/add')).text()).toContain('review: requested → approved');
  });
  ...
  test('the site and the widgets render for a synced project', async () => {
    ...
    for (const [path, expected] of [['/acme/app', 'todo add appends an item'], ['/acme/app', 'built by its own agent'], ['/acme/app/about', 'built by its own agent'], ['/acme/app', 'todo init makes a store'], ['/acme/app?view=list&sort=title', 'abcdef1'], ['/acme/app/items/shipped-1', 'acme.atlassian.net/browse/ACME-12'], ['/acme/app?view=releases', 'v0.1.0'], ['/acme/app?view=timeline', 'September 2026'], ['/acme/app/sessions', '/sessions/run-1'], ['/acme/app/sessions/run-1', 'new EventSource('], ['/acme/app/items/add', 'It appends.']]) {
