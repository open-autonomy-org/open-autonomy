...
export type TestEnv = Env & { ns: MemoryNamespace; gateway: FakeGateway; worker: ExportedHandler<Env> };
export function testEnv(gateway?: Partial<FakeGateway>, app: App = {}): TestEnv {
  ...
  return {
    ...
    GITHUB_TOKEN: 'test-github-org-token',
...
// An app's fakes, by origin: tried before the core's; a handler that returns undefined falls through.
export const fakes = new Map<string, (req: Request, url: URL) => Promise<Response | undefined> | Response | undefined>();
export function useEnv(env: TestEnv): TestEnv { current = env; github.files = {}; github.repos = {}; github.milestones = {}; stripe.requests = []; stripe.cards = {}; stripe.decisions = []; return env; }
globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
  const req = new Request(input, init);
  const url = new URL(req.url);
  const own = fakes.get(url.origin);
  if (own) { const answered = await own(req.clone(), url); if (answered) return answered; }
...
export async function request(env: Env, path: string, init: { method?: string; headers?: Record<string, string>; body?: unknown } = {}): Promise<Response> {
  ...
  return (env as TestEnv).worker.fetch!(new Request(`https://platform.test${path}`, { method: init.method ?? (body ? 'POST' : 'GET'), headers, body }), env, ctx);
}
