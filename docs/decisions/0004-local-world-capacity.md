...
record; proposed until both occur. The upstream World implementation landed separately as
[volter-ai/twin PR 348](https://github.com/volter-ai/twin/pull/348) (merge `eec38833`); that landing is
neither acceptance of this record nor a general package release. A local Evidence Desk recovery ran on a
pinned build of it; those are observations, recorded in the pull request, not acceptance.

## Context and authority

On September 11, 2026, the owner asked why Evidence Desk would not start, challenged the World
reservation system, and directed: "design the good solution", then "go" (coding conversation on the owner's
machine under the owner's login, September 11, 2026; no public permalink; these quotations record scope,
not independent approval). Under the project communication agreement, direction given in a coding session
on the owner's machine is verified direction from that login; the session is its link. The "go" was read as
authorizing implementation and local recovery, not production deployment or disruption of other runs.
