# ADR 0013: A project sustains itself on members, backers and users
...
**Amended** in the same conversation, after the first implementation landed: "make sure we didn't over engineer by
the way - the idea is the have an elegant core"; "let's go over everything again - because I want to remove all over
engineer/prompting". This record states the smaller core: the agents read the roster, the books and the repository
through the doors that already exist.
...
**Before a release, someone who did not build it follows the quick start**, literally, in the project's World.
...
**Asks go to whoever can act.** Work anyone could take goes to a member with the role, one available now first, and
is posted as help-wanted when no one holds it. Work that needs a permission (a repository setting, a release, the
roster) goes to whoever holds that permission, the owner included when only they do; it is never posted as
help-wanted, since no one else could do it. Every ask is prepared so it costs one decision and is sent once. Finding
people for the roles the owner names as needing them is one standing roadmap outcome.

**Strategy owns the plan; the outreach jobs and the community desk carry it out.** The strategy skill gains a
sustain mandate (which users, which channels, which asks). The cadences, the channels and which asks are allowed
are in the project's own document, the owner's word; the agents never edit them.

Extrapolation, this author's (the owner named the goal and the three contributions, not these particulars): the
weekly and monthly jobs; the quick start followed before a release; the release trigger; routing asks by role,
window and permission; strategy as the owner of the plan; the roster as the
...
- **What a person owes is derived, never journaled.** `evidence-desk obligations` computes what each person owes from
  ...
  when met, and lists the acts no one owns in one shared issue. Asks here work the same way: from the roles and
  windows in the roster, one issue each, and the unowned ones as help-wanted.
...
**Given back to it:**
- The roster's roles feed Evidence Desk's import, so its people register is read from the project
...
- `pm`: prepares releases after shipped change; one standing recruiting outcome.
- `strategy`: the sustain mandate and the plan it produces.
- `front-door`: the quick start followed before a release by someone who did not build it.
- `project-communications`: the one statement of how to ask people, and the owner's word on sustaining the project
  (roles and which need people, channels, allowed asks), and the roster as the agents read it.
- The roster (`team` in `.open-autonomy/config.yaml`, the SDK's team codec) gains each member's roles, contributions
  and availability windows, under the same strict validation (SDK 4.0.0); the Team page shows and edits them.
- `community.ts issue open` with no assignee is the help-wanted post.
- The page's tiers keep stating the runway the money buys, from the metered burn.
- The project page drew the monthly total and each patron's amount to viewers the books did not admit, and the front
  page's and a name's project cards drew each listed project's monthly figure to everyone; all of them now draw money
  only where the books are open, as the books panel and the core's own card already did.

## Verification

In the World: an ask routed to a member with the role and, with none, posted as help-wanted; a quick start that does
not work caught before a release; a release prepared after a user-visible change and announced; the backer report
naming a twin patron. Live, on Hookline: the first week's post.
